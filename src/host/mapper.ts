// EventMapper: normalized agy events -> StreamChunk protocol (contract from
// dsh-llm section 3.3). Obligations honored here: usage precedes finish and
// nothing follows it; one content block open at a time; repeated step updates
// grow text by suffix-delta so both snapshot and delta payload styles stream
// correctly.
//
// v1 native tool mirroring: a COMPLETED agy tool step cuts the span — the
// mapper emits a tool-call block addressed to the agy_tool mirror and
// finishes with reason "tool-calls". The gateway protocol layer surfaces the
...
// spans address the mirror tool directly).
import { CallId, type StreamChunk, type TokenUsage } from './stream-types.ts'
...
export function usageFromRaw(raw: RawUsage): TokenUsage {
  ...
  // serialization of chunks stays lossless.
...
export interface EventMapperOptions {
  ...
  runId: string
  /** Cut spans on completed tool steps (main turns). False for internal calls. */
  cutOnTool: boolean
   ...
   * conversation-cumulative and must never reach the gateway's token meter.
   */
  usage?: {
...
export class EventMapper {
   ...
   * back out of the request message list.
   */
  *map(ev: AgyEvent, absIndex: number): Generator<StreamChunk> {
    ...
    if (ev.kind === 'step') {
      ...
      if (ev.stepKind === 'tool' && ev.tool) {
        ...
        if (!this.opts.cutOnTool) return // non-main calls show no tool detail
        // Cut the span: close any open block, then one tool-call block
        // addressed to the mirror tool. The callId encodes the (run,
        // eventIndex) cursor for continuation detection.
        const close = this.closeOpen()
        if (close) yield close
    ...
    if (!ev.ok) {
      ...
      // response stays passive here — the engine owns terminal failures
