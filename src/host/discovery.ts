...
// Ported from dsh-agy-link src/host/discovery.ts @ 46984db (modified: env
// prefix DSH_AGY_CONVERSATIONS_DIR → AGY_PROXY_CONVERSATIONS_DIR).
import { readdirSync, statSync, existsSync } from 'node:fs'
import { join } from 'node:path'
import { homedir } from 'node:os'

export function defaultConversationsDir(): string {
  return process.env.AGY_PROXY_CONVERSATIONS_DIR ?? join(homedir(), '.gemini', 'antigravity-cli', 'conversations')
}
