...
// directly from Google Antigravity backend (v1internal:retrieveUserQuotaSummary
// and v1internal:fetchAvailableModels).
...
export class QuotaService {
  ...
  async refreshAccountQuota(account: ManagedAccount, force = false): Promise<Partial<Record<ModelFamily, FamilyQuotaInfo>> | null> {
    ...
    // the latest email from logs to immediately catch account switching.
    ...
    // Manual refresh: the TOKEN is the only authoritative identity anchor.
    // Log scraping lies — observed live: the slot was labeled
