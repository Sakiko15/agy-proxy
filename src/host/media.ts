// Multimodal staging: inbound image blocks are written to a local
// media directory and referenced by absolute path in the agy prompt —
// agy (an IDE agent) reads them with its own file/vision tools. Stage to
// disk, reference by path, sweep on a TTL. No bytes ever leave the machine
// except through agy itself.
// Ported from dsh-agy-link src/host/media.ts @ 46984db (verbatim).

import { mkdir, readdir, stat, unlink, writeFile } from 'node:fs/promises'
import { join } from 'node:path'

/** Structural image reference accepted by the stager. */
...
/** Byte reader seam — the protocol layer injects how to fetch image bytes. */
...
export async function stageImages(opts: {
  ...
  if (budget > 0 && opts.images.length > 0) {
    ...
    for (const ref of opts.images) {
      ...
      if (!data) {
        skipped++
        lines.push('[image unavailable: "' + label + '" could not be read]')
        i++;
        continue;
      }
...
/** Default media directory: <state dir>/media. */
