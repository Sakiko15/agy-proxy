...
- **优雅停机**：SIGTERM → 停止接新 → 等待在跑 agy（带上限，默认 25s 经 `AGY_PROXY_SHUTDOWN_GRACE_MS` 可调，S2；docker `stop_grace_period: 40s` 留足余量）→ kill 残留进程组 → SQLite checkpoint → 关闭 DB（preClose 先落账本缓冲行）
