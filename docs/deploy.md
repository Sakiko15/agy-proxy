...
compose 的 `image` 行已内置默认 `ghcr.io/sakiko15/agy-proxy:0.2.0`（仍可用 `.env` 的 `AGY_PROXY_IMAGE` 覆盖）。镜像由 GitHub Actions 的 `docker-release` workflow（手动 dispatch，版本入参；`GITHUB_TOKEN` 自带授权推送，**无 registry secrets**）构建，tag 为 `<version>` + `latest`（linux/amd64）。**首次推送后包默认私有**：GitHub → Your packages → agy-proxy → Package settings → Danger Zone → Change visibility → **Public**（一次性，之后 VPS 匿名拉取）。CN VPS 直连 ghcr.io 可能不通或极慢（`registry-mirrors` 加速只代理 Docker Hub、不覆盖 ghcr）——不通时退回路径 A。
...
1. **镜像就位**：GitHub 仓库 Actions 页 → `docker-release` → Run workflow（版本入参与 `package.json` 一致，当前 `0.2.0`）→ 等待构建推送完成 → 把包设为 Public（GitHub → Your packages → agy-proxy → Package settings → Danger Zone → Change visibility，一次性），否则拉取 401/denied。
