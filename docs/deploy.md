...
compose 的 `image` 行已内置默认 `ghcr.io/sakiko15/agy-proxy:latest` 且 `pull_policy: always`——每次 `docker compose up -d` 都会重新拉取最新发布镜像（`restart` 不会重拉；仍可用 `.env` 的 `AGY_PROXY_IMAGE` 覆盖，ghcr 不通时指向本地构建镜像，见路径 A）。镜像由 GitHub Actions 的 `docker-release` workflow（手动 dispatch，版本入参；`GITHUB_TOKEN` 自带授权推送，**无 registry secrets**）构建，tag 为 `<version>` + `latest`（linux/amd64）。**首次推送后包默认私有**：GitHub → Your packages → agy-proxy → Package settings → Danger Zone → Change visibility → **Public**（一次性，之后 VPS 匿名拉取）。CN VPS 直连 ghcr.io 可能不通或极慢（`registry-mirrors` 加速只代理 Docker Hub、不覆盖 ghcr）——不通时退回路径 A。
...
1. **镜像就位**：GitHub 仓库 Actions 页 → `docker-release` → Run workflow（版本入参与 `package.json` 一致，当前 `0.3.1`）→ 等待构建推送完成 → 把包设为 Public（GitHub → Your packages → agy-proxy → Package settings → Danger Zone → Change visibility，一次性），否则拉取 401/denied。
2. **面板创建编排**：1Panel → 容器 → 编排 → 创建编排 → 来源「编辑」→ 文件夹名称填 `agy-proxy` → 粘贴仓库的 `docker-compose.yml` → 确认创建（`image` 默认 `ghcr.io/sakiko15/agy-proxy:latest`，无需 .env；要覆盖镜像或设 `AGY_PROXY_API_KEY` 时才放 `.env`）。
3. **首启密码**：编排详情 → 容器 → 日志（或 SSH 后 `docker logs agy-proxy | grep -i password`）——随机管理密码只打印一次；常驻部署建议直接设 `AGY_PROXY_ADMIN_PASSWORD`（env）。
4. **拉取不通时**（CN VPS）：`registry-mirrors` 镜像加速只代理 Docker Hub、不覆盖 ghcr.io——直连 ghcr 不通/极慢时退回路径 A（VPS 上 `docker build`，自包含、无 Node 前置）。
...
镜像内已内置两道边界（无需配置）：`CMD` 携带 `--max-old-space-size=512`
（soak 实测 RSS 峰值 ≈226MB，512MB 留 >2× 余量；放 `CMD` 而非 `NODE_OPTIONS`
是因为后者会被每个 agy 子进程继承，静默压缩上游 CLI 的堆）；compose 已配
`logging` json-file 轮转（10m × 3）。建议再加容器级内存上限，防 agy 子进程
异常膨胀拖垮宿主机（VPS 常见 1–2GB，留出系统与反代份额）：

```yaml
    mem_limit: 1g # 容器含 agy 子进程的总上限；按 VPS 内存酌情调整
```

注意 `mem_limit` 管的是**整棵进程树**（含每请求 spawn 的 agy），512MB 的堆上限
只约束网关自身——两者维度不同，建议都设。
