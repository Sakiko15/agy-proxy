...
- **验证优先，禁止无据假设**：遇到含糊概念、缺失上下文或陌生逻辑时，先检索代码库与 docs/（移植认知另查 dsh-agy-link 上游）；项目内不足时做 web 检索。分析结论必须基于检索到的事实并注明证据；查不到就明说「未核实/待验证」，绝不用猜测补位
- 与 §5 闭环呼应：对上游行为的认知变更（新事件形状、新错误文本）必须落到证据（官方文档/抓包/桩断言），不接受"应该是这样"的口头推断
