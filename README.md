...
| `AGY_PROXY_USAGE_RETENTION_DAYS` | `0` (keep forever) | Usage-ledger row retention (B4/S8): the hourly housekeeping sweep deletes ledger rows older than this many full local-midnight days. `0` keeps every row (pre-S8 behavior). Not admin-UI writable — reboot-effective ops tuning; the dashboard's day-aggregations are unaffected either way. |
