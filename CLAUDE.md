...
M0 done (skeleton). M1 done: engine layer ported (716fc3f), Fastify service layer + OpenAI non-streaming `/v1/chat/completions` + static `AGY_PROXY_API_KEY` auth + OA1 golden case (d3bf836, 06342e9). M2 done: dual-protocol streaming SSE (hijack + heartbeat + disconnect abort), Anthropic `/v1/messages` + count_tokens + model routes (dual-shape /v1/models with header sniff, /v1/anthropic/models with pagination), widened OpenAI surface (tools accepted-not-executed, assistant tool_calls history, data: images, OA8 discovered-catalog 404, OA10 proportional truncation), 26 golden cases (oa1–oa10, an1–an10, ma1–ma3). M3 done: SQLite storage (db.ts schema v1 WAL+FULL), key management (sha256+prefix keys, plaintext shown once), usage ledger (request-id idempotent, 1s buffered flush), auth hook v2 (RPM + day budgets pre-engine), admin API (CIDR/session/CSRF guards, pool + auth-flow + QR + keys + usage routes), full index.ts wiring (pool + quota poller + shutdown teardown), engine hardening (POOL_EXHAUSTED 429 + Retry-After, VALIDATION_REQUIRED 403 + validation_url, per-account p-queue serialization, enriched onRun + EngineCall.meta). 292 tests. Next: M4 WebUI (§9 pages + SSE push — rides the same admin routes). Track in acceptance.md §3 DoD checklists.
